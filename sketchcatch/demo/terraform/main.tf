...
resource "aws_ecs_task_definition" "task_fixed_template_fargate_container_app" {
  ...
  container_definitions = "[{\"name\":\"api\",\"image\":\"public.ecr.aws/docker/library/nginx:1.27-alpine\",\"essential\":true,\"entryPoint\":[\"/bin/sh\",\"-c\"],\"command\":[\"printf '%s\\\\n' 'server {' '  listen 8080;' '  default_type text/plain;' '  location = /health { return 200 ok; }' '  location / { return 200 SketchCatch-deployment-smoke; }' '}' > /etc/nginx/conf.d/default.conf && exec nginx -g 'daemon off;'\"],\"portMappings\":[{\"containerPort\":8080,\"hostPort\":8080,\"protocol\":\"tcp\"}],\"environment\":[{\"name\":\"PORT\",\"value\":\"8080\"},{\"name\":\"WEB_ORIGIN\",\"value\":\"https://${aws_cloudfront_distribution.cdn_web.domain_name}\"},{\"name\":\"INSTANCE_ID\",\"value\":\"fargate\"}],\"logConfiguration\":{\"logDriver\":\"awslogs\",\"options\":{\"awslogs-group\":\"/ecs/audience-live-check-api\",\"awslogs-region\":\"ap-northeast-2\",\"awslogs-stream-prefix\":\"api\"}}}]"
